...
class BTree {
 ...
 public:
  ...
  int height() {
   if (root == nullptr) return 0; //Es lo mismo que si solo tuviera el nodo raíz, pero los test sugieren esto: no puede con M=3 haber 7 valores diferentes en el árbol
  	//y que la altura esperada sea 1. Por lo que solo con el nodo raíz la altura también debe ser 0
   int counter = 0;
   auto curr = root; while (!curr->leaf) {curr = curr->children[0]; ++counter;}
   return counter;
...
private:
  ...
  static void build_recursive(BTree<T>* tree, vector<T>& vec, int left, int right) {
   if (left==right) {tree->insert(vec[left]); return;}
   if (right-left==1){tree->insert(vec[left]); tree->insert(vec[right]); return;}
...
private:
  bool check_properties_node(Node<TK>* node, int bottomcap, int topcap, bool usebottomcap, bool usetopcap,
   ...
   if (node == nullptr) {cout << "node evaluated is null\n"; return false;}
   if (node == root && root->leaf) {
    TK rootprevkey;
    for (int i=0; i<node->count; ++i) {
     if (i != 0 && rootprevkey >= node->keys[i]) {cout << "internal root keys not ordered\n"; return false;}
     rootprevkey = node->keys[i];
    } return true;
   }
   //compara height (altura propia) con globalheight (altura de la primera hoja encontrada)
   if (node->leaf && globalheight != -1 && height != globalheight) {cout << "different leaf heights\n"; return false;}
   ...
   for (int i=0; i<node->count; ++i) {
    ...
    if (!node->leaf && node->children[i] == nullptr) {cout << "node does not have all possible children\n"; return false;}
    //el hijo izquierdo del nodo i debe cumplir con estar dentro de los valores y todas las propiedades (llamada recursiva)
    if (i == 0) {
     if (!node->leaf && !check_properties_node(node->children[i], bottomcap, node->keys[i], usebottomcap, true,
      globalheight, height+1)) return false;
    } else {
     if (!node->leaf && !check_properties_node(node->children[i], node->keys[i-1], node->keys[i], true, true,
      globalheight, height+1)) return false;
    }
    //los elementos deben estar ordenados ascendentemente sin repetidos en un mismo nodo
    if (i != 0 && prevkey >= node->keys[i]) {cout << "internal non-root node keys not ordered\n"; return false;}
    prevkey = node->keys[i];
   }
   if (!node->leaf) {
    //verificación de que cumpla el último hijo
    if (node->children[node->count] == nullptr) {cout << "last child is nullptr\n"; return false;}
