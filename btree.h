...
private:
	...
	void borrow_from_prev(Node<TK>* node, int idx) {
	    ...
	    if (!child->leaf) { child->children[0] = sibling->children[sibling->count]; }
	...
	void borrow_from_next(Node<TK>* node, int idx) {
	    ...
	    if (!child->leaf) { child->children[child->count + 1] = sibling->children[0]; }
	...
	void merge(Node<TK>* parent, int idx) {
	    ...
	    for (int i = 0; i < right->count; ++i) { left->keys[oldLeftCount + 1 + i] = right->keys[i]; }
	    if (!left->leaf) {
			for (int i = 0; i <= right->count; ++i) { left->children[oldLeftCount + 1 + i] = right->children[i]; }
		}
	    left->count = oldLeftCount + right->count + 1;
	    for (int i = idx; i < parent->count - 1; ++i) { parent->keys[i] = parent->keys[i + 1]; }
	    for (int i = idx + 1; i < parent->count; ++i) { parent->children[i] = parent->children[i + 1]; }
	    parent->count--;
	...
	void fill(Node<TK>* node, int idx) {
    	if (idx > 0 && node->children[idx - 1]->count > min_keys()) { borrow_from_prev(node, idx); }
	    else if (idx < node->count && node->children[idx + 1]->count > min_keys()) { borrow_from_next(node, idx); }
	    else {
...
public:
	...
	void remove(TK key) {
	    ...
	    n = max(0, n - 1);
	    if (root->count == 0) { remove_root_if_empty(); }
	}
